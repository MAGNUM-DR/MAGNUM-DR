...
async function startServer() {
  const app = express();
  const PORT = 3000;
  const MEDIA_FILE = path.join(process.cwd(), 'site-media.json');

  app.use(express.json({ limit: '50mb' }));
  app.use(express.urlencoded({ extended: true, limit: '50mb' }));

  // Permanent Media Storage API for Logos and Package Photos
  app.get('/api/site-media', (_req, res) => {
    try {
      ...
      console.error('Error reading site-media.json:', err);
    }
    res.json({ venueLogo: null, designerLogo: null, packageImages: {} });
  });

  app.post('/api/site-media', (req, res) => {
    try {
      let currentData: { venueLogo: string | null; designerLogo: string | null; packageImages: Record<string, string[]> } = {
        venueLogo: null,
        designerLogo: null,
        packageImages: {},
      };
      if (fs.existsSync(MEDIA_FILE)) {
        try {
          currentData = JSON.parse(fs.readFileSync(MEDIA_FILE, 'utf-8'));
        } catch {}
      }

      const { venueLogo, designerLogo, packageImages } = req.body;
      const merged = {
        venueLogo: venueLogo !== undefined ? venueLogo : currentData.venueLogo,
        designerLogo: designerLogo !== undefined ? designerLogo : currentData.designerLogo,
        packageImages: packageImages ? { ...currentData.packageImages, ...packageImages } : currentData.packageImages,
      };

      fs.writeFileSync(MEDIA_FILE, JSON.stringify(merged, null, 2), 'utf-8');
      return res.json({ success: true, media: merged });
    } catch (err: any) {
      console.error('Error saving site-media.json:', err);
